...
static int build_exponential_number(long double value, int precision,
                                    ...
                                    int *exponent_result, FormatSpec *spec) {
...
int s21_format_g(char *str, FormatSpec *spec, long double value,
                 int uppercase) {
  int precision = 6;
...
static long long get_int_value(FormatSpec *spec, va_list *args, int is_signed) {
    long long value = 0;
    if (spec->length == 'l') {
...
static long double get_float_value(FormatSpec *spec, va_list *args) {
  long double value;
  if (spec->length == 'L') {
